...
exports.handler = async (event) => {
  ...
  if (!gender || !ageRange || !ethnicity || !skin) {
    return { statusCode: 400, body: JSON.stringify({ success: false, error: 'Please complete the Creator Stylist profile first.' }) };
  }
  ...
  try {
    ...
    const prompt = `You are a master AI image prompt architect for influencer content. Write a complete influencer image prompt package. Ready to use in Midjourney, Leonardo, OpenArt or Kling.\n\nSUBJECT:\n- Gender: ${gender}\n- Age: ${ageRange}\n- Ethnicity: ${ethnicity}\n- Skin: ${skinDesc}\n- Hair: ${hairDesc || 'natural'}\n- Outfit: ${outfit || 'stylish'}\n- Setting: ${location || 'studio'}\n- Aesthetic: ${aesthetic || 'clean and modern'}\n- Feeling: ${goal || 'confident'}\n- Platform: ${platform || 'Instagram'}${category ? `\n- Shot type: ${category}` : ''}${extra ? `\n- Extra: ${extra}` : ''}${trendContext ? `\n\nTrend context: ${trendContext}` : ''}\n\nWrite THREE sections:\n\n1. MAIN IMAGE PROMPT\nOne detailed paragraph ready to use as an AI image prompt. Include subject description, outfit, expression, pose, setting, lighting, camera angle, lens, depth of field, colour grade, mood.\n\n2. SCENE DESCRIPTION\n2-3 sentences describing the full scene for a photographer or art director.\n\n3. STYLING NOTES\n4 bullet points of specific styling details to get exactly right.\n\nNo generic outputs. Every detail specific to this person.`;
